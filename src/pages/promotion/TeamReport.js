...
export default function TeamReport(){
  ...
  const [filteredReports, setFilteredHistory] = useState([]);
  ...
  const [error, setError] = useState(null);
    ...
    useEffect(() => {
      ...
      const month = `month${yesterday.getMonth() + 1}`; // No leading zero for month
      const day = `date${yesterday.getDate()}`; // No leading zero for day
  
      // Set the state values
      setIsYear(String(year));
      setIsMonth(month);
      setIsDate(day);
    }, []); // Empty dependency array to run only on component mount

    const fetchTeamRecords = async (level = -1, date = null) => {
      try {
          const response = await Api.post('/api/webapi/listTotalTeamDeposits', {
              level,  // Pass the level here
              date    // Optionally pass the date here
          });
          const data = response.data;
  
          setTeamRecord(data);  // Update the state with fetched data
      } catch (err) {
  ...
  useEffect(() => {
    const fetchTeamReport = async () => {
      try {
        ...
        setFilteredHistory(data.teamReports); // Initialize filteredReports with all data
      } catch (err) {
        console.error('An error occurred:', err);
        setError('An error occurred. Please try again.');
      }
  ...
  const displayTier = () => {
    return isTier === 'All' ? 'All'
      ...
      : isTier === 'tier6' ? 'Tier 5'
      : isTier === 'tier7' ? 'Tier 6'
      : 'Unknown Tier'; // Fallback for unhandled cases
  };

  const displayLevel = () => {
    return isTier === 'All' ? null
      : isTier === 'tier2' ? 1
      : isTier === 'tier3' ? 2
      : isTier === 'tier4' ? 3
      : isTier === 'tier5' ? 4
...
return formattedFullDate;
  ...
  const handleConfirm = () => {
    let filtered = teamReport;

    let date = displayDate();

    if(isTier === 'All'){
      fetchTeamRecords(null,date);

    }
  
    if (isTier === 'tier2') {
      filtered = teamReport.filter((history) => history.level === 1);
      fetchTeamRecords(1,date);

    } else if (isTier === 'tier3') {
      filtered = teamReport.filter((history) => history.level === 2);
      fetchTeamRecords(2,date);


    } else if (isTier === 'tier4') {
      filtered = teamReport.filter((history) => history.level === 3);
      fetchTeamRecords(3,date);


    }else if (isTier === 'tier5') {
      filtered = teamReport.filter((history) => history.level === 4);
      fetchTeamRecords(4,date);


    } else if (isTier === 'tier6') {
      filtered = teamReport.filter((history) => history.level === 5);
      fetchTeamRecords(5,date);


    } else if (isTier === 'tier7') {
      filtered = teamReport.filter((history) => history.level === 6);
      fetchTeamRecords(6,date);
...
fetchTeamRecords(level, formattedFullDate);  // Fetch for level 3 and a specific date


console.log(formattedFullDate);
// Filter the history data based on the formatted date  
 filteredData = teamReport.filter((history) => formatDate1(history.updated_at) === formattedFullDate);
        ...
        <div data-v-10d1559c="" className="TeamReport__C-head">
          <div data-v-10d1559c="" className="TeamReport__C-head-fixed">
            ...
            <div data-v-10d1559c="" className="TeamReport__C-head-line2">
              <div data-v-10d1559c="">
                <span data-v-10d1559c="" className="default">{displayTier()}</span
                ><i
                  data-v-10d1559c=""
              ...
              <div data-v-10d1559c="">
                <span data-v-10d1559c="" className="default">{displayDate()}</span
                ><i
                  data-v-10d1559c=""
      ...
      <div className="van-picker-column">
        ...
        >
          ...
          <li
            role="button"
            tabindex="0"
            className="van-picker-column__item"  id="month8" onClick={()=>month('month8')}
            style={{height: '44px'}}
          >
